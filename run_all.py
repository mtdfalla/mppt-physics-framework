...
What this does (in order):
    ...
    Step 3  — Publication figures (9 PNG files, figures/)
...
# ── Step 5: Temperature study ────────────────────────────────────────────────
def run_temperature_study():
    import subprocess
    for T in (45, 65):
        for psc in ('easy', 'moderate', 'hard', 'extreme'):
            out = f'results/study_temperature_T{T}_{psc}.json'
            if os.path.exists(out):
                print(f"  Skipping {out} (already exists)")
                continue
            subprocess.run([sys.executable, 'study_temperature.py',
                            '--T', str(T), '--psc', psc], check=True)


# ── Step 6: Fast-transition study ────────────────────────────────────────────
def run_fast_study():
    import subprocess
    out = 'results/study_fast_transitions.json'
    if os.path.exists(out):
        print(f"  Skipping {out} (already exists)")
        return
    subprocess.run([sys.executable, 'study_fast_transitions.py'], check=True)


# ── Step 7: Timing benchmark ─────────────────────────────────────────────────
def run_bench():
    import subprocess
    subprocess.run([sys.executable, 'bench_decision_time.py'], check=True)
...
if __name__ == '__main__':
    ...
    step("Step 1/7 — Run 96-case simulation matrix", run_simulations)
    step("Step 2/7 — Compute gap metrics (RMSE, response time, EN50530)", run_gaps)
    step("Step 3/7 — Generate publication figures", run_figures)
    step("Step 4/7 — Generate manuscript tables", run_tables)
    step("Step 5/7 — Temperature study (45 and 65 C, all PSC levels)", run_temperature_study)
    step("Step 6/7 — Fast-transition study (b = 0.1 s, 0.25 s, 1 s ramp)", run_fast_study)
    step("Step 7/7 — Per-decision timing benchmark", run_bench)
